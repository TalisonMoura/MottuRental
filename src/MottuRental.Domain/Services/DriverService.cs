    ...
    IHandler<DomainNotification> notifications) : BaseServiceEntity<Driver>(baseRepository, notifications), IDriverService
{
    ...
    {
        var entity = await ExecuteQuery.Where(x => x.Cnpj.Equals(driver.Cnpj) || x.NumeroCNH.Equals(driver.NumeroCNH) || x.Name.Equals(driver.Name)).FirstOrDefaultAsync();

        return entity is null ? await RegisterAsync(driver) : default;
    }
}