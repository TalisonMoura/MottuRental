    ...
    IHandler<DomainNotification> notifications) : BaseServiceEntity<Driver>(baseRepository, notifications), IDriverService
{
    ...
    {
        var entity = await ExecuteQueryAsNoTracking.AnyAsync(x => x.Cnpj.Equals(driver.Cnpj) || x.NumeroCNH.Equals(driver.NumeroCNH) || x.Name.Equals(driver.Name), cancellationToken);

        return !entity ? await RegisterAsync(driver, cancellationToken) : default;
    }
}